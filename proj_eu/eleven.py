...
M = [[int(col) for col in row]for row in L]
...
for i in range(17):
  for z in range(17):
     #right/left
     product = M[i][z] * M[i][z+1] * M[i][z+2] * M[i][z+3]
     print 'prod %s %s %s %s' %(M[i][z], M[i][z+1], M[i][z+2], M[i][z+3])
     if product > greatest_product:
       greatest_product = product
     #up/down
     product = M[z][i] * M[z+1][i] * M[z+2][i] * M[z+3][i]
     print 'prod %s %s %s %s' %(M[z][i], M[z+1][i], M[z+2][i], M[z+3][i])
     if product > greatest_product:
       greatest_product = product
...
for i in range(17):
  for x in range(17):
    #diagonal
    product = M[i][x] * M[i+1][x+1] * M[i+2][x+2] * M[i+3][x+3]
    print 'prod %s %s %s %s' %(M[i][x], M[i+1][x+1], M[i+2][x+2], M[i+3][x+3])
    if product > greatest_product:
      greatest_product = product
